...
@app.route('/api/crimes/sample', methods=['GET'])
def crimes_sample():
    if request.method == 'GET':
        try:
            ...
            payload = [build_crimes(r) for r in cursor]
            success = True
            code = 200
...
@app.route('/api/crimes/by/<group_id>/<hour>/<day_of_week>/<month>/<grid_id>', methods=['GET'])
def crimes_by(group_id, hour, day_of_week, month, grid_id):
    if request.method == 'GET':
        try:
            connection = psycopg2.connect(
                user="postgres",
            ...
            cursor.execute("""
                ...
                WHERE 
                    ...
                    month = %s AND 
                    grid_id = %s;
            """ % (group_id, hour, day_of_week, month, grid_id))
...
@app.route('/api/crimes/predict/<hour>/<day_of_week>/<month>/<grid_id>', methods=['GET'])
